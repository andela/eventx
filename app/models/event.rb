class Event < ActiveRecord::Base
  ...
  def self.popular_events
    find_by_sql(PrepareSearchQuery.prepare_popular_query)
  end

  def self.search(title = "", location = "", date = "", category_id = "")
    query = PrepareSearchQuery.prepare_query(title, location, date, category_id)
    date_range = []
    date_range = DateFormatter.new.format_date(date) unless date.empty?
    find_by_sql [query, {
